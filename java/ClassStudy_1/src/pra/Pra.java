...
public class Pra {
  ...
  public String title(Board b1, Board b2){
    return b1.getBoardNum() > b2.getBoardNum() ? b1.getTitle() : b2.getTitle();
  }
  //매개변수로 게시글 객체 두개가 전달되면
  //두 게시글의 조회수의 합을 리턴하는 메서드
  public int readCntSum(Board b1, Board b2){
    return b1.getReadCnt() + b2.getReadCnt();
  }

  //매개변수로 글 번호, 제목이 전달되면
  //해당 데이터로 갖는 게시글 객체를 리턴하는 메서드
  public Board bo(int boardNum, String title){
    Board b =  new Board(7, "study","me", 77777);
    return b;
  }

  //매개변수로 두개의 게시글 객체가 전달되면
  //두 객체 중 글번호가 큰 게시글을 리턴하는 메서드
  public Board test(Board b1, Board b2){
    return b1.getBoardNum() > b2.getBoardNum() ? b1 : b2;
    //게시글 자체를 리턴하는 것이니 b1, b2 전체 게시글 리턴할 수 있게 함.
  }
